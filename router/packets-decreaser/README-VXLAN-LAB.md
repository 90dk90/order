...
| RSS multi-queue on `x520wan` | **Blocked by PPPoE**: NIC sees ethertype `0x8864`, `rss=0x0` → 100% `rx_q0`. Use `pd-rss-tune.sh` (wan: poll q0 only). |
| `x520lan` RX mode | **Poll q0–q3**. Interrupt on lan q1–q3 silently drops hashed return frames → VPS↛`/24`, Digi BVI ping loss, customer outbound blackhole. |
| VPS → `79.172.242.2` | **OK** once lan queues poll (no Digi NAT hairpin needed). `.3`/`.10` still down on host. |
...
## Soft handoff after PPPoE (Option B) — deployed (1 restart)
