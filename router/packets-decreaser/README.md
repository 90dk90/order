...
CAKE shaping is **off** (`PD_SHAPE=0`) — max Digi/GRE throughput, no artificial cap.
Scripts `pd-gre-shape-cake.sh` / `pd-pve-upload-cake.sh` remain available but unused.
