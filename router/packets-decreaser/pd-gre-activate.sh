#!/bin/sh
# PD GRE primary path — all dedicated LAN egress/ingress via VPP gre0.
# Soft/idempotent: never del/add storms (those SIGSEGV VPP 26.06 FIB).
# Sticky/ABF/classify abandoned. Proximus = admin only.
set -eu
. /etc/pd/pd-gre.conf
...
[ -n "$SRC" ] || { echo "pd-gre: no Digi WAN IPv6 yet — watchdog will retry"; exit 0; }
...
if [ -x /usr/local/sbin/pd-lan-prepare.sh ]; then
  ...
  $VPP set interface l2 bridge x520lan "$LAN_BD" 2>/dev/null || true
...
NEED_GRE=0
CUR_SRC=""
if ! $VPP show gre tunnel 2>/dev/null | grep -q .; then
  NEED_GRE=1
else
...
if [ "$NEED_GRE" = 1 ]; then
  # Recreate only when src/dst wrong — single del then create (no FIB storms)
...
if ! $VPP show interface addr gre0 2>/dev/null | grep -q "${INNER_LOCAL%%/*}"; then
...
# Default VRF via gre0 — add if missing only
...
# Strip sticky ACL only if leftover state file exists (never probe ACL 0 blindly)
...
# Harden optional — default skip on soft path (ACL churn). Opt-in: PD_SKIP_HARDEN=0
if [ "${PD_SKIP_HARDEN:-1}" = 0 ] && [ -x /usr/local/sbin/pd-gre-harden.sh ]; then
